...
Module QuotientFilterProbability (Spec:  QuotientFilterSpec).
  ...
  Lemma quotientfilter_add_mutliple_query_preserve qf qf' v hashes xs hshs':
    (quotientfilter_query_internal v qf) ->
    ((d[ quotientfilter_add_multiple hashes qf xs]) (hshs', qf')) != 0 ->
    (quotientfilter_query_internal v qf').
   Proof.
  ...
  Theorem quotientfilter_no_false_negatives l hashes qf  x xs :
    ...
    uniq (x :: xs) -> length xs == l -> hash_has_free_spaces (l.+1) hashes  ->
    all (fun x => hash_unseen x hashes) (x::xs) ->
    (d[ res1 <-$ quotientfilter_add x hashes qf;
          let '(hsh1, bf1) := res1 in
          res2 <-$ quotientfilter_add_multiple hsh1 bf1 xs;
            let '(hsh2, bf2) := res2 in
            res3 <-$ quotientfilter_query x hsh2 bf2;
  ...
  Proof.
    ...
    rewrite{1}/hash {1}/hashstate_find Hunsx; comp_normalize.
    exchange_big_outwards 2 => //=; comp_simplify_n 1.
    exchange_big_outwards 2 => //=; comp_simplify_n 1.
    exchange_big_outwards 4 => //=; comp_simplify_n 1.
    ...
    - move/eqP: Hllen Hlen => ->; case: xs Huniq Hunsxs => [//=|//=] Huniq Hunsxs; move=> Hllen; comp_normalize.
      comp_simplify; rewrite/hash; move: Hfree; rewrite/hash_has_free_spaces addnS -addSn -ltnS => /addr_ltn.
      rewrite -[[length hashes].+1]addn1 ltnS => H.
      under eq_bigr do under eq_bigr do under eq_bigr => i1 _ do rewrite (hash_find_insert_involutive _ _ _ _ H).
      comp_normalize; comp_simplify.
    ...
    - under eq_bigr => qf' _. {
        ...
        rewrite addnS; move=>/fixedlist_add_incr =>/(_ ind'' x) => Hlength Hfree Hshs.
        have Huns':  all ((hash_unseen (n:=n))^~ (fixmap_put x ind'' hashes)) xs. {
          apply/allP => v Hv; move: Huniq => //=/andP[/memPn /(_ v Hv)] => Hneq _ .
            by move/allP: Hunsxs => /(_ v Hv)/(fixmap_find_neq) => /(_ x ind'' Hneq).
        }
        have Hlen': [length hashes] + 1 <= n; first
                                                                                    by move/eqP: Hllen Hfree; clear; case: l => //= l _; rewrite addnS -addSn => /addr_ltn/ltnW; rewrite addn1.
        move/eqP: (@hash_find_insert_involutive _ x ind'' hashes Hlen') => Hfind'; rewrite/hash.
